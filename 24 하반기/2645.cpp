...
int k, m;
/*
1. 꺾이는 지점만 주어졌을 때 그래프를 어떻게 완성시킬 것인가?
-> 꺾이는 지점은 x, y 둘 중 하나만 달라지니까 최솟값 ~ 최댓값에 항상 마킹을하는 방식으로 해결
2. 꺾이는 지점만 어떻게 체크할 것인가?
-> 체크할 지점의 방향(초기화는 아예 상관없는 값인 -1)이 달라질 경우에만 answer 벡터에 넣는 식으로 해결
그 외는 다익스트라의 형태. 단 시작점의 거리가 0이 아닌 1로 시작하는 것에 유의하기
